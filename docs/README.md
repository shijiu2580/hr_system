...
# 首次运行（安装依赖、迁移数据库、初始化 RBAC 示例角色）
.\scripts\start_dev.ps1 -Install
```

说明：当前脚本会优先使用项目内虚拟环境与 `uv`，并优先调用 D 盘的 npm 运行时。
...
docker-compose exec backend python manage.py init_rbac_permissions --with-roles
...
#    或 python manage.py init_rbac_permissions --with-roles --force
...
当前仓库已提供 Docker Compose 和 Nginx 配置；Kubernetes 与 Render 产物未包含在仓库中，如需这些部署方式请额外补充对应清单。
