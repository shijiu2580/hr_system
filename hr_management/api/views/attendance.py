...
class AttendanceCheckAPIView(views.APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if action == 'check_in':
                if not rec:
                    if current_time > late_cutoff and not notes.strip():
                        return Response(api_error('签到已超过 09:00，请填写迟到原因', code='reason_required'), status=400)
                    att_type = 'late' if current_time > late_cutoff else 'check_in'
                    # 迟到时添加标签
                    formatted_notes = f'迟到原因：{notes}' if (current_time > late_cutoff and notes) else notes
                    rec = Attendance(employee=emp, date=today, attendance_type=att_type, check_in_time=current_time, notes=formatted_notes)
                    rec.save()
                    log_event(user=request.user, action='签到', detail=f'{emp.employee_id} {today}', ip=get_client_ip(request))
                else:
                    ...
                    if current_time > late_cutoff and rec.attendance_type == 'check_in':
                        rec.attendance_type = 'late'
                    if notes:
                        # 迟到时添加标签
                        rec.notes = f'迟到原因：{notes}' if current_time > late_cutoff else notes
                ...
                is_early_leave = current_time < early_leave_cutoff
