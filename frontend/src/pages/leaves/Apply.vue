  ...
  <div class="page-container">
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="header-title">我的休假</span>
      </div>
      <button class="btn-apply" @click="router.push('/leaves/create')">请假</button>
    </div>
      ...
      <table class="data-table">
        ...
        <tbody v-if="!loading && paged.length">
          <tr v-for="item in paged" :key="item.id" class="data-row">
            <td class="col-type">
              <a href="javascript:;" class="type-link">{{ getLeaveTypeLabel(item.leave_type) }}</a>
            </td>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="detail-row">
            ...
            <span class="detail-value">{{ getLeaveTypeLabel(logItem.leave_type) }}</span>
          </div>
          <div class="detail-row">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <textarea v-model="editForm.reason" class="form-textarea" rows="4" placeholder="请填写请假原因"></textarea>
          </div>
        </div>
        ...
        <div class="modal-body">
          <div class="confirm-message">
            ...
            <p class="confirm-detail">
              {{ getLeaveTypeLabel(cancelItem.leave_type) }}：{{ cancelItem.start_date }} 至 {{ cancelItem.end_date }}
            </p>
          </div>
        </div>
        <div class="modal-footer">
...
function openCancel(item) {
  ...
  if (!isBeforeStartDate(item)) {
    message.value = { type: 'error', text: '假期已经开始，无法销假' }
...
async function submitEdit() {
  if (!editForm.value.start_date || !editForm.value.end_date || !editForm.value.leave_type) {
...
async function submitCancel() {
  submitting.value = true
  try {
    const resp = await api.post(`/leaves/${cancelItem.value.id}/cancel/`)
    if (resp.success) {
      message.value = { type: 'success', text: '销假成功' }
      cancelItem.value = null
      load()
    } else {
      message.value = { type: 'error', text: resp.error?.message || '销假失败' }
    }
  } catch (e) {
    message.value = { type: 'error', text: '操作失败' }
  } finally {
...
async function load() {
  ...
  if (resp.success) {
    ...
    console.error('加载请假数据失败:', resp.error)
    message.value = {
      type: 'error',
